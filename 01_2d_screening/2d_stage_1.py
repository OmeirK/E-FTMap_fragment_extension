...
def mp_func(mp_plan):
    ...
    plan = mp_plan[1]
    #output = {'anchor_1': [], 'anchor_2' : [], 'anchor_3' : []}
    output = {}
    for a in plan:
        output[a] = []
            ...
            if mol:
                for name, patt_dict in plan.items():
                    ...
                    if len(anchor_matches) > 0:
                        output[name].append((smiles, idnumber))
...
with open(args.smarts_list) as f:
    ...
    for line in f:
        ...
        if patt:
            plan[f'anchor_{idx}'] = {'anchor_patt' : patt}
            idx += 1
        else:
...
for name in plan:
    ...
    for output in outputs:
        for smiles, idnumber in output[name]:
            file_lines.append(f'{smiles}\t{idnumber}')
